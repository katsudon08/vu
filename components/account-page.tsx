...
import { Pencil, Check, X, ChevronRight, Trophy, Flame } from "lucide-react"
...
export function AccountPage() {
  ...
  const myActivities = store.activities.filter((a) => a.userId === store.currentUserId)
  ...
  return (
    <div className="flex-1 p-4 space-y-6 pb-24">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <div className="gradient-border rounded-full p-[3px]">
              <div className="w-20 h-20 rounded-full bg-background flex items-center justify-center overflow-hidden">
                <img
                  src={store.currentUserAvatar || "/placeholder.svg"}
                  alt="アバター"
          ...
          <div className="flex-1 space-y-1">
            {isEditing ? (
              ...
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-bold">{store.currentUserName}</h2>
                <button
      ...
      <div className="card-gradient rounded-2xl p-6">
        <h3 className="text-sm font-medium text-muted-foreground mb-4">よく実行するジャンル</h3>
        {mostFrequentCategory ? (
          <div className="flex items-center gap-4">
            <div
              className={cn(
                "w-20 h-20 rounded-2xl bg-gradient-to-br flex items-center justify-center text-4xl shadow-lg",
                categoryIcons[mostFrequentCategory]?.color || "from-gray-400 to-gray-500",
              )}
            >
              {categoryIcons[mostFrequentCategory]?.icon || "✨"}
            </div>
            <div>
              <p className="text-2xl font-bold">{mostFrequentCategory}</p>
              <p className="text-muted-foreground text-sm">
                {myActivities.filter((a) => a.category === mostFrequentCategory).length}回実行
              </p>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-lg font-medium text-muted-foreground">まだアクティビティがありません</p>
              <p className="text-sm text-muted-foreground">アクティビティを実行してみましょう</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="card-gradient rounded-2xl p-5">
          <div className="flex items-center gap-2 text-muted-foreground mb-2">
            ...
            <span className="text-sm font-medium">累計実行回数</span>
          </div>
          <p className="text-3xl font-black gradient-instagram-text">
            {activityCount}
            <span className="text-lg font-medium text-muted-foreground ml-1">回</span>
          </p>
        </div>
        <div className="card-gradient rounded-2xl p-5">
          <div className="flex items-center gap-2 text-muted-foreground mb-2">
          ...
          {nextRank ? (
            <>
              ...
              <p className="text-xs text-muted-foreground mt-1">→ {nextRank.name}</p>
            </>
          ) : (
            <p className="text-lg font-bold text-yellow-500">最高ランク達成!</p>
          )}
        </div>
      </div>
      ...
      {nextRank && (
        <div className="card-gradient rounded-2xl p-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm font-medium text-muted-foreground">{nextRank.name}</span>
          </div>
          <div className="h-3 bg-secondary rounded-full overflow-hidden">
            <div
              className={cn("h-full rounded-full bg-gradient-to-r transition-all duration-500", currentRank.color)}
              style={{
                width: `${((activityCount - currentRank.minCount) / (nextRank.minCount - currentRank.minCount)) * 100}%`,
              }}
            />
          </div>
      ...
      {/* 最近のアクティビティ履歴 */}
      <div className="card-gradient rounded-2xl p-5">
        <h3 className="text-sm font-medium text-muted-foreground mb-4">最近のアクティビティ</h3>
        {myActivities.length > 0 ? (
          <>
                ...
                >
                  <div
                    className={cn(
                      ...
                      categoryIcons[activity.category]?.color || "from-gray-400 to-gray-500",
                    )}
                  >
                    {categoryIcons[activity.category]?.icon || "✨"}
                  </div>
            ...
            {myActivities.length > 3 && (
              ...
              >
                {showAllHistory ? "閉じる" : `もっと見る (${myActivities.length - 3}件)`}
                <ChevronRight className={cn("w-4 h-4 transition-transform", showAllHistory && "rotate-90")} />
              </button>
            )}
          </>
        ) : (
          <p className="text-center text-muted-foreground py-8">まだアクティビティがありません</p>
        )}
      </div>
    </div>
  )
